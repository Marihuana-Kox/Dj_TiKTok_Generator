...
from article.models import Article, ArticleCluster  # Или твоя модель статьи
...
class AudioProject(models.Model):
    ...
    provider = models.CharField("Провайдер", max_length=50, default="replicate_f5tts")
    ...
    # Аудиофайл (итоговый)
    audio_file = models.FileField("Аудиофайл", upload_to="audio_projects/", null=True, blank=True)
...
class AudioTrack(models.Model):
    ...
    # Связи
    project = models.ForeignKey(AudioProject, on_delete=models.CASCADE, related_name="tracks")
    ...
    # Результат
    audio_file = models.FileField("Аудиофайл", upload_to="audio_tracks/", null=True, blank=True)
    status = models.CharField("Статус", max_length=20, choices=STATUS_CHOICES, default="pending")
    error_message = models.TextField("Ошибка", blank=True)
