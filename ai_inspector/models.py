...
class AIProvider(models.Model):
    ...
    provider_type = models.CharField("Тип", max_length=20, choices=PROVIDER_TYPES)

    api_key = models.TextField("API Key (Зашифрован)", blank=True, editable=True)
    ...
    def clean(self):
        if not self.name.islower() or " " in self.name:
            raise ValidationError("Кодовое имя должно быть в lowercase без пробелов.")
