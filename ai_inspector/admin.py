...
class AIProviderAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk and self.instance.api_key:
            self.fields["api_key_input"].widget.attrs["placeholder"] = "******** (ключ установлен)"
...
class AIProviderAdmin(admin.ModelAdmin):
    ...
    list_display = ["name", "display_name", "provider_type", "is_active", "created_at"]
    list_filter = ["provider_type", "is_active"]
    search_fields = ["name", "display_name"]
    readonly_fields = ["created_at", "updated_at"]

    fieldsets = (
        ("Основное", {"fields": ["name", "display_name", "provider_type", "is_active"]}),
        (
            "Доступ",
            {
        ...
        (
            "Настройки модели",
            {"fields": ["config"], "description": 'JSON: {"model": "gpt-4o", "temperature": 0.7}'},
        ),
