document.addEventListener('DOMContentLoaded', function() {
    // 1. Находим форму. Убедись, что в HTML у тега <form> есть id="generate-form"
    const form = document.getElementById('generate-form');
    if (!form) {
        console.warn("Форма с id='generate-form' не найдена");
        return;
    }
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            console.log("✅ Ответ от сервера:", data); // Смотри в консоль браузера (F12)
            ...
            if (data.status === 'ok' && data.task_id) {
                window.startProgressTracking(
                    ...
                    'progress-modal',         // 3. ID твоего модального окна (совпадает с твоим HTML)
                    function(result) {        // 4. Что делать после завершения (встроено в modal.js)
                ...
                // Если форма невалидна или произошла ошибка
